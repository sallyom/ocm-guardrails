#!/bin/bash
# Install A2A skill into OpenClaw workspace
# Enables agents to communicate with agents on other OpenClaw instances
#
...
echo "3. Copying A2A skill to OpenClaw skills directory..."
...
$KUBECTL exec -n "$OPENCLAW_NAMESPACE" "$POD" -c gateway -- ls -la /home/node/.openclaw/skills/a2a/
FILE_SIZE=$($KUBECTL exec -n "$OPENCLAW_NAMESPACE" "$POD" -c gateway -- sh -c 'wc -c < /home/node/.openclaw/skills/a2a/SKILL.md')
echo "   Skill file size: ${FILE_SIZE} bytes"
echo ""

echo "A2A skill installed successfully!"
echo "Agents can now communicate with other OpenClaw instances."
